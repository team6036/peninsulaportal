// https://github.com/gerth2/NetworkTablesClients/blob/main/nt4/js/src/msgpack/msgpack.js
...
export function serialize(data, options) {
    ...
    if (options.multiple && !util.is(data, "arr")) throw "Invalid arguments: Cannot serialize multiple unless data is an array";
    ...
    let typeHint = "";
    if (options.typeHint) typeHint = options.typeHint;

    if (options.multiple) Array.from(data).forEach(d => append(d, false, typeHint));
    else append(data, false, typeHint);

    return arr.subarray(0, l);

    function append(data, isReplacement, typeHint) {
        switch (typeof(data)) {
            case "undefined":
                appendNull(data);
                break;
            ...
            default:
                if (!isReplacement && options.invalidTypeReplacement)
                    append(
                        util.is(options.invalidTypeReplacement, "func") ? options.invalidTypeReplacement(data) : options.invalidTypeReplacement,
                        true,
                        typeHint,
                    );
                else throw "Invalid arguments: data of type "+typeof(data)+" cannot be serialized";
        }
        ...
        function appendNumber(data, typeHint) {
            let isInt = (typeHint == "int") || (util.is(data, "int") && !["float", "double"].includes(typeHint));
            if (isInt) {
                if (data >= 0 && data <= 0x7f)
					appendByte(data);
...
export function deserialize(arr, options) {
    ...
    if (typeof(arr) != "object" || typeof(arr.length) == "undefined")
        throw "Invalid arguments: Expected a byte array (Array or Uint8Array) to deserialize";
    if (!arr.length)
        throw "Invalid argument: The byte array to deserialize is empty";
        ...
        if (c < 2048) {
            ...
            if (c > 0xd7ff && c < 0xdc00) {
                ...
                if (c2 < 0xdc00 || c2 > 0xdfff)
                    throw "UTF-8 encode: second surrogate character 0x"+c2.toString(16)+" at index "+ci+" out of range";
...
function decodeUTF8(bytes, start, l) {
    let i = start, str = "";
    l += start;
    while (i < l) {
        let c = bytes[i++];
        if (c > 127) {
            if (c > 191 && c < 224) {
                if (i >= l) throw "UTF-8 decode: incomplete 2-byte sequence";
                c = (c & 31) << 6 | bytes[i++] & 63;
            }
            else if (c > 223 && c < 240) {
                if (i + 1 >= l) throw "UTF-8 decode: incomplete 3-byte sequence";
                c = (c & 15) << 12 | (bytes[i++] & 63) << 6 | bytes[i++] & 63;
            }
            else if (c > 239 && c < 248) {
                if (i + 2 >= l) throw "UTF-8 decode: incomplete 4-byte sequence";
