...
import { Process, ProcessManager } from "./processes";
import { Client, ClientManager } from "./clients";
import { TBAClient, TBAClientManager } from "./tbaclients";



const OS: util.Immutable<{
    arch: string,
...
let bootParams: util.StringMap<any> = {};
...
const BOOTPARAMS: util.Immutable<util.StringMap<any>> = Object.freeze(bootParams);



function makePureEscape(value: any): string {
    return "\x1B["+value;
}
function makeEscape(...args: number[]): string {
    return makePureEscape(args.join(";")+"m");
}
...
function log(...args: any[]) {
    ...
    console.log(`${yr}-${mon}-${d} ${hr}:${min}:${s}.${ms}`, ...args);
}

async function showError(name: string, type: string | null, e: ErrorEvent | Error | string): Promise<void> {
    let message = name;
    if (type) message += " - "+type;
    electron.dialog.showErrorBox(message, (e == null) ? "" : (typeof(e) === "string") ? e : util.stringifyError(e));
}
async function showWarn(name: string, type: string | null, e: ErrorEvent | Error | string): Promise<void> {
    let message = name;
    if (type) message += " - "+type;
    await electron.dialog.showMessageBox({
        message: message,
        detail: (e == null) ? "" : (typeof(e) === "string") ? e : util.stringifyError(e),
        type: "warning",
        buttons: ["OK"],
    });
}
async function showSuccess(name: string, type: string | null, e: ErrorEvent | Error | string): Promise<void> {
    let message = name;
    if (type) message += " - "+type;
    await electron.dialog.showMessageBox({
        message: message,
...
async function showConfirm(name: string, type: string | null, e: ErrorEvent | Error | string, ok: string = "OK", cancel: string = "Cancel"): Promise<boolean> {
    let message = name;
    if (type) message += " - "+type;
...
async function showTerminationConfirm(name: string, type: string | null, e: ErrorEvent | Error | string) {
    return await showConfirm(name, type, e, "Terminate", "Continue anyway");
}


async function zlibCompress(src: fs.PathLike, dst: fs.PathLike): Promise<boolean> {
...
async function zlibDecompress(src: fs.PathLike, dst: fs.PathLike): Promise<boolean> {
    await new Promise(async (res, rej) => {
...
async function tarCompress(src: fs.PathLike, dst: fs.PathLike): Promise<boolean> {
        ...
        const t = tar.create(
            {
                sync: false,
                gzip: true,
                cwd: path.dirname(String(src)),
                preservePaths: false,
            },
            [path.basename(String(src))],
        );
        t.on("error", rej);
...
async function tarDecompress(src: fs.PathLike, dst: fs.PathLike): Promise<boolean> {
    await new Promise((res, rej) => {
...
function makeMenuDefault(name: string, signal?: util.Target): (electron.MenuItemConstructorOptions | electron.MenuItem)[] {
    if (!(signal instanceof util.Target)) signal = new util.Target();
    name = String(name);
    return [
        {
            label: (name.length > 0) ? lib.getAppName(name) : "Portal",
            submenu: [
                {
                    label:
                        (name.length > 0) ?
                            ("About Peninsula "+lib.getAppName(name)) :
                        "About Peninsula",
                    enabled: !!("about" in signal ? signal.about : true),
                    click: () => (signal as util.Target).post("about"),
                },
                { type: "separator" },
                {
                    label: "Settings",
                    ...
                    enabled: !!("about" in signal ? signal.about : true),
                    click: () => (signal as util.Target).post("settings"),
                },
                { type: "separator" },
                { role: "hide" },
                { role: "hideOthers" },
...
type PPathLike = string | PPathLike[];

class FSOperator extends util.Target {
    private static _fsLogFunc: Function | null;
    static get fsLogFunc() { return this._fsLogFunc; }
    static set fsLogFunc(value) { this._fsLogFunc = value; }
    ...
    static makePath(...pth: PPathLike[]) { return path.join(...util.flattenArray(pth) as string[]); }
    static async fileHas(pth: PPathLike) {
    ...
    static async fileDeny(pth: PPathLike) {
        pth = this.makePath(pth);
    ...
    static async dirHas(pth: PPathLike) {
        pth = this.makePath(pth);
    ...
    static async dirList(pth: PPathLike) {
        pth = this.makePath(pth);
    ...
    static async dirDeny(pth: PPathLike) {
        pth = this.makePath(pth);
    ...
    async fileHas(pth: PPathLike) { return await FSOperator.fileHas([this.root, pth]); }
    async fileRead(pth: PPathLike) { return await FSOperator.fileRead([this.root, pth]); }
    async fileReadRaw(pth: PPathLike) { return await FSOperator.fileReadRaw([this.root, pth]); }
    async fileWrite(pth: PPathLike, content: string) { return await FSOperator.fileWrite([this.root, pth], content); }
    async fileWriteRaw(pth: PPathLike, content: Uint8Array) { return await FSOperator.fileWriteRaw([this.root, pth], content); }
    async fileAppend(pth: PPathLike, content: string) { return await FSOperator.fileAppend([this.root, pth], content); }
    async fileDelete(pth: PPathLike) { return await FSOperator.fileDelete([this.root, pth]); }
    async fileAffirm(pth: PPathLike, content: string = "") { return await FSOperator.fileAffirm([this.root, pth], content); }
    async fileDeny(pth: PPathLike) { return await FSOperator.fileDeny([this.root, pth]); }

    async dirHas(pth: PPathLike) { return await FSOperator.dirHas([this.root, pth]); }
    async dirList(pth: PPathLike) { return await FSOperator.dirList([this.root, pth]); }
    async dirMake(pth: PPathLike) { return await FSOperator.dirMake([this.root, pth]); }
    async dirDelete(pth: PPathLike) { return await FSOperator.dirDelete([this.root, pth]); }
    async dirAffirm(pth: PPathLike) { return await FSOperator.dirAffirm([this.root, pth]); }
    async dirDeny(pth: PPathLike) { return await FSOperator.dirDeny([this.root, pth]); }

    static fsLog(...args: any[]) { if (this.fsLogFunc) this.fsLogFunc(...args); }
}
...
class Window extends util.Target {
    ...
    readonly clientManager;
    readonly tbaClientManager;
    ...
    private _menu: electron.Menu | null;
    public perm: boolean;

    public state: any;

    constructor(manager: WindowManager, name: string) {
        super();
    ...
    hasWindow(): boolean { return !!this.window && !this.window.isDestroyed() && !this.window.webContents.isDestroyed(); }
    get menu() { return this._menu; }

    async getPerm(): Promise<boolean> {
        if (!this.started) return true;
        this.log("GET PERM");
        let perm: boolean = await new Promise((res, rej) => {
            ...
            const permack = (e: electron.IpcMainEvent) => {
                if (!this.window) return;
                if (!this.hasWindow()) return;
                if (e.sender.id !== this.window.webContents.id) return;
                clear();
            };
            const perm = (e: electron.IpcMainEvent, given: any) => {
                if (!this.window) return;
                if (!this.hasWindow()) return;
                if (e.sender.id !== this.window.webContents.id) return;
                clear();
        ...
        let namefs: util.StringMap<() => boolean> = {
        };
        if (this.name in namefs) perm &&= namefs[this.name]();
        return perm;
    }

    start(params: any = null): boolean {
        ...
        const onHolidayState = async (holiday: string | null) => {
            let tag = "png" as const;
            let defaultIcon = path.join(__dirname, "..", "assets", "app", "icon."+tag);
            let icon = 
                (holiday == null) ? defaultIcon :
                lib.castHolidayIcon(lib.castHolidayIcons(await this.get("holiday-icons"))[holiday])[tag];
            if (!this.window) return;
            if (!this.hasWindow()) return;
        ...
        const readiness: number = 2000;
        let id = setTimeout(async () => {
            clear();
            if (this.isModal) return;
            let result = await showTerminationConfirm(
            ...
            if (result || !this.window || !this.hasWindow()) return this.stop();
            // ew
            ready({ sender: { id: this.window.webContents.id } } as electron.IpcMainEvent);
            this.window.webContents.openDevTools();
        }, readiness);
        const clear = () => {
            clearInterval(id);
        ...
        const ready = (e: electron.IpcMainEvent) => {
            if (!this.window) return;
            if (!this.hasWindow()) return;
            if (e.sender.id !== this.window.webContents.id) return;
            clear();
        ...
        let anyHasDevToolsOpen = false;
        for (let window of this.manager.windows) {
            if (!window.hasWindow()) continue;
            if (!window.window.webContents.isDevToolsOpened()) continue;
            anyHasDevToolsOpen = true;
            break;
        }
        if (anyHasDevToolsOpen) this.window.webContents.openDevTools();
        this.window.webContents.on("devtools-opened", () => {
            this.manager.windows.filter(window => window.hasWindow()).forEach(window => window.window.webContents.openDevTools());
        });
        this.window.webContents.on("devtools-closed", () => {
            this.manager.windows.filter(window => window.hasWindow()).forEach(window => window.window.webContents.closeDevTools());
        });
        ...
        namefs = {
            ...
            PANEL: async () => {
                ...
                client.id = "logger";
                client.addHandler("stream-logs", async (fname: string, payload: any, meta: any, ssStream: fs.ReadStream) => {
                    await this.affirm();
        ...
        let finished = false, messageQueue: [string, ...any[]][] = [["init", params]];
        const checkMessages = () => {
            if (!finished) return;
            if (!this.window) return;
            if (!this.hasWindow()) return;
            while (messageQueue.length > 0)
                this.window.webContents.send("message", ...messageQueue.shift() as [string, ...any[]]);
        };
        const addMessage = (name: string, ...args: any[]) => {
            messageQueue.push([name, ...args]);
            checkMessages();
        };
        ...
        (async () => {
            ...
            if (!this.window) return;
            if (!this.hasWindow()) return;
            let size = this.window.getSize() as util.vec2;
            const finishAndShow = () => {
                if (!this.window) return;
                if (!this.hasWindow()) return;
                this.resolver.state++;
    ...
    async stop(): Promise<boolean> {
        ...
        if (this.window && this.hasWindow()) this.window.close();
        this._window = null;
        this._menu = null;
    ...
    static getDataPath(manager: WindowManager, name: string, started: boolean = true): string | null {
    ...
    static getCanOperate(manager: WindowManager, name: string, started: boolean = true): boolean {
        return lib.FEATURES.includes(name) && started;
    }
    get canOperate() { return Window.getCanOperate(this.manager, this.name, this.started); }

    static async affirm(manager: WindowManager, name: string, started: boolean = true): Promise<boolean> {
    ...
    async affirm(): Promise<boolean> { return await Window.affirm(this.manager, this.name, this.started); }

    static async fileHas(manager: WindowManager, name: string, pth: PathLike, started: boolean = true): Promise<boolean> {
        if (!this.getCanOperate(manager, name, started)) return false;
    ...
    static async fileRead(manager: WindowManager, name: string, pth: PathLike, started: boolean = true): Promise<string | null> {
        if (!this.getCanOperate(manager, name, started)) return null;
        await this.affirm(manager, name, started);
        return await WindowManager.fileRead(WindowManager.makePath(this.getDataPath(manager, name, started), pth));
    }
    static async fileReadRaw(manager: WindowManager, name: string, pth: PathLike, started: boolean = true): Promise<Uint8Array | null> {
        if (!this.getCanOperate(manager, name, started)) return null;
        await this.affirm(manager, name, started);
        return await WindowManager.fileReadRaw(WindowManager.makePath(this.getDataPath(manager, name, started), pth));
    }
    static async fileWrite(manager: WindowManager, name: string, pth: PathLike, content: string, started: boolean = true): Promise<void> {
        if (!this.getCanOperate(manager, name, started)) return;
        await this.affirm(manager, name, started);
        return await WindowManager.fileWrite(WindowManager.makePath(this.getDataPath(manager, name, started), pth), content);
    }
    static async fileWriteRaw(manager: WindowManager, name: string, pth: PathLike, content: Uint8Array, started: boolean = true): Promise<void> {
        if (!this.getCanOperate(manager, name, started)) return;
        await this.affirm(manager, name, started);
        return await WindowManager.fileWriteRaw(WindowManager.makePath(this.getDataPath(manager, name, started), pth), content);
    }
    static async fileAppend(manager: WindowManager, name: string, pth: PathLike, content: string, started: boolean = true): Promise<void> {
        if (!this.getCanOperate(manager, name, started)) return;
        await this.affirm(manager, name, started);
        return await WindowManager.fileAppend(WindowManager.makePath(this.getDataPath(manager, name, started), pth), content);
    }
    static async fileDelete(manager: WindowManager, name: string, pth: PathLike, started: boolean = true): Promise<void> {
        if (!this.getCanOperate(manager, name, started)) return;
        await this.affirm(manager, name, started);
        return await WindowManager.fileDelete(WindowManager.makePath(this.getDataPath(manager, name, started), pth));
    }
    static async fileAffirm(manager: WindowManager, name: string, pth: PathLike, content: string = "", started: boolean = true): Promise<void> {
        if (!this.getCanOperate(manager, name, started)) return;
        await this.affirm(manager, name, started);
        return await WindowManager.fileAffirm(WindowManager.makePath(this.getDataPath(manager, name, started), pth), content);
    }
    static async fileDeny(manager: WindowManager, name: string, pth: PathLike, started: boolean = true): Promise<void> {
        if (!this.getCanOperate(manager, name, started)) return;
        await this.affirm(manager, name, started);
        return await WindowManager.fileDeny(WindowManager.makePath(this.getDataPath(manager, name, started), pth));
    }

    static async dirHas(manager: WindowManager, name: string, pth: PathLike, started: boolean = true): Promise<boolean> {
        if (!this.getCanOperate(manager, name, started)) return false;
        await this.affirm(manager, name, started);
        return await WindowManager.dirHas(WindowManager.makePath(this.getDataPath(manager, name, started), pth));
    }
    static async dirList(manager: WindowManager, name: string, pth: PathLike, started: boolean = true): Promise<fs.Dirent[]> {
        if (!this.getCanOperate(manager, name, started)) return [];
        await this.affirm(manager, name, started);
        return await WindowManager.dirList(WindowManager.makePath(this.getDataPath(manager, name, started), pth));
    }
    static async dirMake(manager: WindowManager, name: string, pth: PathLike, started: boolean = true): Promise<void> {
        if (!this.getCanOperate(manager, name, started)) return;
        await this.affirm(manager, name, started);
        return await WindowManager.dirMake(WindowManager.makePath(this.getDataPath(manager, name, started), pth));
    }
    static async dirDelete(manager: WindowManager, name: string, pth: PathLike, started: boolean = true): Promise<void> {
        if (!this.getCanOperate(manager, name, started)) return;
        await this.affirm(manager, name, started);
        return await WindowManager.dirDelete(WindowManager.makePath(this.getDataPath(manager, name, started), pth));
    }
    static async dirAffirm(manager: WindowManager, name: string, pth: PathLike, started: boolean = true): Promise<void> {
        if (!this.getCanOperate(manager, name, started)) return;
        await this.affirm(manager, name, started);
        return await WindowManager.dirAffirm(WindowManager.makePath(this.getDataPath(manager, name, started), pth));
    }
    static async dirDeny(manager: WindowManager, name: string, pth: PathLike, started: boolean = true): Promise<void> {
        if (!this.getCanOperate(manager, name, started)) return;
        await this.affirm(manager, name, started);
        return await WindowManager.dirDeny(WindowManager.makePath(this.getDataPath(manager, name, started), pth));
    }

    async fileHas(pth: PathLike): Promise<boolean> { return Window.fileHas(this.manager, this.name, pth, this.started); }
    async fileRead(pth: PathLike) { return Window.fileRead(this.manager, this.name, pth, this.started); }
    ...
    async getThis(k, ...a) {
        ...
        let kfs = {
            ...
            "project": async id => {
                await this.onThis("projects-affirm");
                id = lib.formatKeyString(lib.sanitize(id));
                const root = await this.getThis("root");
                let content = null;
                try {
    ...
    async onThis(k, ...a) {
        ...
        let kfs = {
            ...
            "project-export": async (pthDst, id) => {
                ...
                const pthSrc = WindowManager.makePath(await this.getThis("root"), "projects", lib.formatKeyString(lib.sanitize(id))+".json");
            ...
            "read": async (type, pth) => {
                ...
                if (type == "ds") {
                    ...
                    let [logData, eventsData] = await Promise.all([logPath, eventsPath].map(async pth => await WindowManager.fileReadRaw(pth)));
                    return {
                        logData: logData,
    ...
    async send(k, ...a) {
        ...
        k = String(k);
        this.log(`SEND - ${k}(${a.map(v => lib.truncateString(JSON.stringify(v))).join(', ')})`);
        if (!this.hasWindow()) return false;
...
const MAIN = async function(this: util.StringMap<any>) {
    class WindowManager extends FSOperator {
        #window;
        ...
        async tryLoad() {
            ...
            let r = await (async () => {
                ...
                const fetchAndPipe = async (url, pth) => {
                    ...
                    let resp = await util.timeoutMillis(30000, fetch(url));
                ...
                await Promise.all(assetsMap.map(async key => {
                    ...
                    const id = lib.formatKeyString(lib.sanitize(key.shift()));
                    const name = key.join(".");
                    let typefs = {
                        themes: async () => {
            ...
            let kfs = {
                ...
                "active-theme": async (type=null) => {
                    ...
                    if (DATATYPES.includes(type)) data = await kfs._writable([type, "themes", "config.json"], "active");
                        ...
                        for (let type of DATATYPES) data = lib.mergeValues(data, await kfs["active-theme"](type));
                    }
                ...
                "active-template": async (type=null) => {
                    ...
                    if (DATATYPES.includes(type)) data = await kfs._writable([type, "templates", "config.json"], "active");
                        ...
                        for (let type of DATATYPES) data = lib.mergeValues(data, await kfs["active-template"](type));
                    }
                    return (data == null) ? null : String(data);
                },
                ...
                "robots": async (type=null) => {
                    ...
                    if (DATATYPES.includes(type)) {
                        ...
                        for (let type of DATATYPES) data = lib.mergeValues(data, await kfs["robots"](type));
                    }
                    return util.ensure(data, "obj");
                },
                ...
                "active-robot": async (type=null) => {
                    ...
                    if (DATATYPES.includes(type)) data = await kfs._writable([type, "robots", "config.json"], "active");
                        ...
                        for (let type of DATATYPES) data = lib.mergeValues(data, await kfs["active-robot"](type));
                    }
                    return (data == null) ? null : String(data);
                },
                ...
                "holidays": async (type=null) => {
                    ...
                    if (DATATYPES.includes(type)) {
                        ...
                        for (let type of DATATYPES) data = lib.mergeValues(data, await kfs["holidays"](type));
                    }
                    return util.ensure(data, "obj");
                },
                ...
                "socket-host": async (type=null) => {
                    ...
                    if (DATATYPES.includes(type)) data = await kfs._writable([type, "config.json"], "socketHost");
                        ...
                        for (let type of DATATYPES) data = lib.mergeValues(data, await kfs["socket-host"](type));
                    }
                ...
                "scout-url": async (type=null) => {
                    ...
                    if (DATATYPES.includes(type)) data = await kfs._writable([type, "config.json"], "scoutURL");
                        ...
                        for (let type of DATATYPES) data = lib.mergeValues(data, await kfs["scout-url"](type));
                    }
                    return (data == null) ? null : String(data);
                },
                "comp-mode": async (type=null) => {
                    ...
                    if (DATATYPES.includes(type)) data = await kfs._writable([type, "config.json"], "isCompMode");
                        ...
                        for (let type of DATATYPES) data = lib.mergeValues(data, await kfs["comp-mode"](type));
                    }
                    return !!data;
                },
                "native-theme": async (type=null) => {
                    ...
                    if (DATATYPES.includes(type)) data = await kfs._writable([type, "config.json"], "nativeTheme");
                        ...
                        for (let type of DATATYPES) data = lib.mergeValues(data, await kfs["native-theme"](type));
                    }
                ...
                "holiday-opt": async (type=null) => {
                    ...
                    if (DATATYPES.includes(type)) data = await kfs._writable([type, "config.json"], "holidayOpt");
                        ...
                        for (let type of DATATYPES) data = lib.mergeValues(data, await kfs["holiday-opt"](type));
                    }
                    return !!data;
                },
                "reduced-motion": async (type=null) => {
                    ...
                    if (DATATYPES.includes(type)) data = await kfs._writable([type, "config.json"], "reducedMotion");
                        ...
                        for (let type of DATATYPES) data = lib.mergeValues(data, await kfs["reduced-motion"](type));
                    }
                    return !!data;
                },
        ...
        async delThis(k, ...a) {
            ...
            let kfs = {
                ...
                "template-image": async id => {
                    await this.fileDelete(["override", "templates", lib.formatKeyString(lib.sanitize(id)), "image.png"]);
                    this.rootManager.check();
                },
                "template-model": async id => {
                    await this.fileDelete(["override", "templates", lib.formatKeyString(lib.sanitize(id)), "model.glb"]);
                    this.rootManager.check();
                },
                ...
                "robot-default": async id => {
                    const robot = util.ensure(await this.getThis("robot", id), "obj");
                    await this.fileDelete(["override", "robots", lib.formatKeyString(lib.sanitize(id)), lib.sanitize(robot.default || "model")+".glb"]);
                    this.rootManager.check();
                },
                "robot-component": async (id, k) => {
                    ...
                    k = String(k);
                    if (!(k in components)) return;
                    await this.fileDelete(["override", "robots", lib.formatKeyString(lib.sanitize(id)), lib.sanitize(k)+".glb"]);
                    this.rootManager.check();
                },
        ...
        async onThis(k, ...a) {
            ...
            let kfs = {
                ...
                "_export": async (k, pthDst, id, type=null) => {
                    k = lib.formatKeyString(lib.sanitize(k));
                    ...
                    const pthSrc = WindowManager.makePath(this.dataPath, source, k, lib.formatKeyString(lib.sanitize(id)));
                ...
                "_rekey": async (k, idSrc, idDst) => {
                    k = lib.formatKeyString(lib.sanitize(k));
                    await fs.promises.rename(
                        WindowManager.makePath(this.dataPath, "override", k, lib.formatKeyString(lib.sanitize(idSrc))),
                        WindowManager.makePath(this.dataPath, "override", k, lib.formatKeyString(lib.sanitize(idDst))),
                    );
                },
                "_make": async (k, id, o) => {
                    k = lib.formatKeyString(lib.sanitize(k));
